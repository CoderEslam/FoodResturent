package com.doubleclick.foodresturent.android.views.EasingInterpolator
...
internal object EasingProvider {
    ...
    private fun getBounceOut(elapsedTimeRate: Float): Float {
        ...
        return if (elapsedTimeRate < 1 / 2.75) {
            ...
            (7.5625 * (1.5 / 2.75.let { (elapsedTimeRate.minus(it)).toFloat(); elapsedTimeRate }) * elapsedTimeRate + 0.75).toFloat()
        } else if (elapsedTimeRate < 2.5 / 2.75) {
            (7.5625 * (2.25 / 2.75.let { (elapsedTimeRate.minus(it)).toFloat(); elapsedTimeRate }) * elapsedTimeRate + 0.9375).toFloat()
        } else {
            (7.5625 * (2.625 / 2.75.let { (elapsedTimeRate.minus(it)).toFloat(); elapsedTimeRate }) * elapsedTimeRate + 0.984375).toFloat()
        }
    }
